...
\subsection{The one-move coercion game}
\label{app:game-onemove}

\noindent \S\ref{sub:nga} states this game in prose; here it is in full.

\begin{definition}[Coercion game and coercion-safety]\label{def:game}
Fix a scheme $\Sigma$ and a priced adversary $\mathrm{WDY}[W,c]$. In
$\mathbf{Coerce}_{\Sigma}$: (1)~$\Attacker$ mounts an encounter
(\ref{d:subjugation}--\ref{d:noperceptual}), reaching a state with a set $R$ of
\emph{racers} --- the released $\Victim$ and any remote delegate --- and $p_A(S)$
is its probability of prevailing against racer set $S$ (monotone: fewer racers,
weakly higher $p_A$), so Definition~\ref{def:racers}'s scalar is $p_A(R)$.
(2)~$\Attacker$ picks $a \in \{\textsf{spare}, \textsf{kill}\}$, where
$\textsf{kill}$ removes $\Victim$ from $R$ --- by reachability it can eliminate
only racers it can \emph{reach}, never a remote delegate. (3)~The race resolves,
with payoff
\[
...
\]
for every WDY adversary $\Attacker'$.
\end{definition}
