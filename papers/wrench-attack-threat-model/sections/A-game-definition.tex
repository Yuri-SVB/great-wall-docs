...
familiar ``the adversary reaches $\Kset$,'' and a \textbf{deadly-race game}, whose
...
can trigger even when it provably cannot reach $\Kset$.
...
the \emph{money}, seeks $\Kset$, and would kill only \emph{instrumentally}, where the
...
reach $\Kset$?} The deadly-race game instead posits a \emph{hypothetical} adversary
...
\begin{enumerate}[nosep,leftmargin=1.7em]
  ...
  producing the spend authority $\Kset$ (\S\ref{sub:setup}), the co-located device and explicit-secret state
  ...
  $p_A = p_A(R)$ the probability $\mathcal{X}$ then reaches $\Kset$
...
$\Attacker$ feasibly reaches $\Kset$ --- equivalently $p_A > 0$
(Definition~\ref{def:racers}). $\Pi$ is \textbf{seizure-resistant} if
\[
  \Pr\!\big[\SzExp_{\Pi}(\Attacker) = 1\big] = 0
\]
for every WDY adversary
$\Attacker$.
\end{definition}
...
the floor, so no feasible path to $\Kset$ exists and $p_A = 0$; hence
$\SzExp$ outputs $0$. On the residual seizure window $\SzExp$ may output $1$ --- the
$o_N$-live coincidence where $\Attacker$ does reach $\Kset$; the loss there is of
...
The \textbf{deadly-race experiment} $\DRExp_{\Pi}(\Attacker')$ runs the interaction
(Definition~\ref{def:wdy-int}) with adversary $\mathcal{X} = \Attacker'$ and holder
...
\[
  \Pr\!\big[\DRExp_{\Pi}(\Attacker') = 1\big] = 0
\]
for every WDY adversary
$\Attacker'$.
\end{definition}
...
part only at \emph{full compromise} $p_A = 1$: the seizure game outputs $1$ (the stash is
...
seizure game (cannot reach $\Kset$; e.g.\ one facing a graceful-recovery vault it cannot
...
$\Pr[\DRExp_{\Pi}(\Attacker') = 1] \le \varepsilon$ for all $\Attacker'$. The claims
...
\end{theorem}
\begin{proof}
$\DRExp$ outputs $1$ exactly on a reachable state with $\pi > 0$, so
...
$\mathsf{Coerce}$, and $\mathsf{Compute}$ leaves $\Kset$ at the floor, so
the reachable endpoint is the \emph{clear failure} $p_A = 0$ --- the opposite
endpoint $p_A = 1$ would require feasibly reaching $\Kset$, which the same theorem
