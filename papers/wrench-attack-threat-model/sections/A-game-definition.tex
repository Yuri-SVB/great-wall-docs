...
full-compromise endpoint $p_A = 1$ is \emph{clear}, not a win: the game penalises
...
--- \emph{on top of} removing the gray race, it minimises the window in which the
stash is seizable at all, via the wipe-before-memory-hard step
of~\eqref{eq:orbit} and the $\sftstar$ floor (Theorem~\ref{thm:floor}).
\end{remark}
...
final orbit point $o_N$ --- currently or imminently materialised --- and
...
$\mathsf{Coerce}$, and $\mathsf{Compute}$ leaves $K$ at least $\sftstar$ away, so
...
timelock-rescue vault advertised as \emph{individual} custody, the adversary that
