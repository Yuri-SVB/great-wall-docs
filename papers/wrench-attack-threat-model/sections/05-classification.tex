...
(accessed 2026-08-14) by keyword; the coding is coarse and a reproducible script
...
Two fielded cases sharpen the classification, and both are analysed in
Appendix~\ref{app:classify}. The 2025 abduction and mutilation of \textbf{David
Balland}~\cite{balland} ended in rescue by an elite \emph{state} police unit and
recovery via a \emph{centralized} stablecoin freeze, with an associate paying the
ransom --- exactly what clause~\ref{d:nodelegatecoerce} assumes away. And the 2026
\textbf{RATIRL} incident~\cite{ratirl}, in which a streamer was tortured for hours
over cryptocurrency he denied holding, is Corollary~\ref{cor:disclosure} in the
field: where the norm is ``deny that you own Bitcoin,'' a coerced denial is
\emph{pre-discredited}, so the rational continuation of an interrogation opening
with one is \emph{more} coercion. It fixes the \emph{sign} --- obscurity does not
merely fail to protect, it can convert a robbery into a torture.
